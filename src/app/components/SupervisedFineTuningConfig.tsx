...
export function SupervisedFineTuningConfig({ model, onValidationChange, onOpenExtensionManagement, enabledExtensions }: SupervisedFineTuningConfigProps) {
  ...
  const orderedExtensions = [...(enabledExtensions ?? [])].sort((a, b) => EXT_STAGE_ORDER.indexOf(a.type) - EXT_STAGE_ORDER.indexOf(b.type));
...
<div style={{ minWidth: 0, flex: 1 }}><b style={{ display: "block", color: C.ink, fontSize: 13, fontWeight: 650 }}>扩展组件</b><span style={{ display: "block", marginTop: 2, color: C.muted, fontSize: 11 }}>已启用的框架扩展按挂载阶段注入本次训练任务，可按任务开关。</span></div>
...
<span style={{ color: on ? C.green : C.faint, fontSize: 10.5, fontWeight: 600, flex: "0 0 auto" }}>{on ? "本次任务注入" : "本次任务不注入"}</span>
