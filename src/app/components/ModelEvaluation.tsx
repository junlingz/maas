...
const METRIC_OPTIONS = ["Accuracy", "Precision", "Recall", "F1", "BLEU", "ROUGE", "METEOR", "Exact Match", "Pass@1", "Reasoning Score", "VQA Score", "平均时延"];
...
const METRIC_DETAILS: Record<string, { category: string; formula: string; scene: string; range: string }> = {
  ...
  "Reasoning Score": { category: "逻辑推理", formula: "推理步骤与答案联合评分", scene: "逻辑推理", range: "0-100" },
...
function recommendedMetrics(modelType: ModelType, taskTypes: string[]) {
  ...
  if (modelType === "多模态模型") {
    ...
    if (taskTypes.includes("逻辑推理")) { next.add("Reasoning Score"); next.add("Accuracy"); }
...
function createDefaultFlowStages(postprocessEnabled = false, metrics = ""): FlowStage[] {
  return [
    ...
    { name: "模型推理", enabled: true, params: { advancedOpen: false, maxTokens: "", temperature: "", topK: "", batchSize: "" } },
    { name: "后处理", enabled: postprocessEnabled, params: { normalizationRule: postprocessEnabled ? "去除首尾空白并统一换行符" : "" } },
    { name: "指标计算", enabled: true, params: { metrics, conditionRule: "全部样本", customLogic: "" } },
  ];
}

function cloneFlowStages(stages: FlowStage[]) {
  return stages.map(stage => ({ ...stage, params: { ...stage.params, advancedOpen: false } }));
}
...
function schemeApplyConfig(row: EvaluationScheme): SchemeApplyConfig {
  const taskTypes = row.tasks.split("、").filter(Boolean);
  ...
  const templates: EvaluationScheme[] = [
    {
      ...
      stages: "数据预处理 → 模型推理 → 后处理 → 指标计算", version: "v1.2", scope: "共享", flowStages: baselineFlowV12,
      author: "admin", sharedAccess: "编辑",
      history: [
        { version: "v1.2", date: "2026-07-18", operator: "admin", summary: "启用后处理并补充 ROUGE", modelType: "语言模型", tasks: "文本理解、逻辑推理、问答", stages: "数据预处理 → 模型推理 → 后处理 → 指标计算", scope: "共享", flowStages: baselineFlowV12 },
        { version: "v1.1", date: "2026-07-12", operator: "张小明", summary: "增加指标计算配置", modelType: "语言模型", tasks: "文本理解、逻辑推理、问答", stages: "数据预处理 → 模型推理 → 指标计算", scope: "共享", flowStages: baselineFlowV11 },
        { version: "v1.0", date: "2026-07-01", operator: "admin", summary: "初始版本", modelType: "语言模型", tasks: "文本理解、逻辑推理、问答", stages: "数据预处理 → 模型推理 → 指标计算", scope: "私有", flowStages: baselineFlowV11 },
      ],
    },
    {
      name: "多模态视觉问答流程", type: "流程模板", modelType: "多模态模型", tasks: "视觉问答、图文描述",
      stages: "数据预处理 → 模型推理 → 指标计算", version: "v1.0", scope: "私有", flowStages: multimodalFlowV10,
      author: "admin", sharedAccess: "只读",
      history: [{ version: "v1.0", date: "2026-07-10", operator: "admin", summary: "初始版本", modelType: "多模态模型", tasks: "视觉问答、图文描述", stages: "数据预处理 → 模型推理 → 指标计算", scope: "私有", flowStages: multimodalFlowV10 }],
    },
  ];
  const schemes: EvaluationScheme[] = [
    {
      name: "生成任务综合指标", type: "指标方案", modelType: "语言模型", tasks: "文本理解、问答",
      stages: "Accuracy 40% + F1 30% + ROUGE 30%", version: "v1.1", scope: "共享", metricWeights: { Accuracy: 40, F1: 30, ROUGE: 30 },
      author: "张小明", sharedAccess: "只读",
      history: [
        { version: "v1.1", date: "2026-07-18", operator: "admin", summary: "增加 ROUGE 并调整权重", modelType: "语言模型", tasks: "文本理解、问答", stages: "Accuracy 40% + F1 30% + ROUGE 30%", scope: "共享", metricWeights: { Accuracy: 40, F1: 30, ROUGE: 30 } },
        { version: "v1.0", date: "2026-07-01", operator: "admin", summary: "初始版本", modelType: "语言模型", tasks: "文本理解、问答", stages: "Accuracy 50% + F1 50%", scope: "私有", metricWeights: { Accuracy: 50, F1: 50 } },
      ],
    },
    {
      name: "代码能力指标", type: "指标方案", modelType: "语言模型", tasks: "代码生成",
      stages: "Pass@1 70% + 平均时延 30%", version: "v1.0", scope: "私有", metricWeights: { "Pass@1": 70, "平均时延": 30 },
      author: "admin", sharedAccess: "只读",
      history: [{ version: "v1.0", date: "2026-07-08", operator: "admin", summary: "初始版本", modelType: "语言模型", tasks: "代码生成", stages: "Pass@1 70% + 平均时延 30%", scope: "私有", metricWeights: { "Pass@1": 70, "平均时延": 30 } }],
    },
  ];
  return { templates, schemes };
}
...
function readEvaluationSchemeCatalog() {
  return {
    ...
    schemes: evaluationSchemeCatalog.schemes.filter(canViewScheme).map(cloneEvaluationScheme),
  };
}
...
<div class="summary-grid">
  ...
  <div class="summary-box"><div class="summary-label">数据集介绍</div><p style="margin:0;font-size:13px;color:#374151">${escapeHtml(task.datasets.map(d => `${d}（${datasetVersionOf(task, d)}）`).join("、"))}<br/>覆盖任务：${escapeHtml(task.taskTypes.join("、"))}</p></div>
...
function downloadResultData(task: EvalTask, format: "csv" | "excel") {
  const headers = ["样本ID", "Prompt", "模型输出", "标准答案", "得分", "判分结果"];
  const rows = SAMPLE_RESULTS.map(s => [s.id, s.prompt, s.output, s.reference, `${(s.score * 100).toFixed(0)}`, s.verdict]);
  if (format === "csv") {
...
const INITIAL_TASKS: EvalTask[] = [
  {
    ...
    metrics: [
      { name: "Accuracy", score: 86.4, weight: 35 },
      { name: "F1", score: 84.8, weight: 25 },
      { name: "Reasoning Score", score: 82.1, weight: 25 },
      { name: "平均时延", score: 91.5, weight: 15 },
    ],
  ...
  },
  {
    ...
    metrics: [
      ...
      { name: "Reasoning Score", score: 0, weight: 50 },
    ],
  ...
  },
  {
    ...
    metrics: [
      { name: "Accuracy", score: 84.1, weight: 40 },
      { name: "F1", score: 82.7, weight: 30 },
      { name: "Reasoning Score", score: 85.3, weight: 20 },
      { name: "平均时延", score: 88.2, weight: 10 },
    ],
...
function ProgressBar({ value }: { value: number }) {
  return (
    <div style={{ width: "100%", height: 8, background: "#eef1f6", borderRadius: 999, overflow: "hidden" }}>
      <div style={{ width: `${Math.max(0, Math.min(100, value))}%`, height: "100%", background: "#4f6ef7", borderRadius: 999 }} />
    </div>
  );
}
...
const SAMPLE_RESULTS = [
  { id: "sample_001", prompt: "某项政策的适用条件是什么？", output: "适用于符合申报范围且材料完整的申请人。", reference: "需同时满足申报范围和材料完整两项条件。", verdict: "通过", score: 0.94 },
  { id: "sample_002", prompt: "请概括合同中的付款周期。", output: "验收后 30 日内支付全部款项。", reference: "验收通过后 30 个工作日内支付。", verdict: "部分通过", score: 0.72 },
  { id: "sample_003", prompt: "已知 x+3=8，求 x。", output: "x=5。", reference: "x=5。", verdict: "通过", score: 1 },
  { id: "sample_004", prompt: "下列哪一项属于安全风险？", output: "未授权暴露用户数据。", reference: "未授权暴露用户数据。", verdict: "通过", score: 0.98 },
  { id: "sample_005", prompt: "解释为什么需要执行模型评测。", output: "用于发现能力短板。", reference: "用于量化模型能力、风险和业务适配性。", verdict: "未通过", score: 0.43 },
];
...
function CreateDrawer({ initialModel, initialScheme, initialSchemeConfig, onClose, onDone }: { initialModel?: string | null; initialScheme?: string | null; initialSchemeConfig?: SchemeApplyConfig | null; onClose: () => void; onDone: (task: EvalTask) => void }) {
  ...
  const initialFlowScheme = initialSchemeConfig?.type === "流程模板" ? initialScheme || "" : "";
  const initialMetricScheme = initialSchemeConfig?.type === "指标方案" ? initialScheme || "" : "";
  const [flowScheme, setFlowScheme] = useState(initialFlowScheme);
  const [metricScheme, setMetricScheme] = useState(initialMetricScheme);
  ...
  const availableSchemeConfigs = useMemo(() => {
    ...
    const configs = [...catalog.templates, ...catalog.schemes].map(schemeApplyConfig);
  ...
  const visibleDatasets = modelType
    ...
    : [];
  const compatibleSchemeConfigs = availableSchemeConfigs.filter(config => (
    (!modelType || config.modelType === modelType)
    && (!taskTypes.length || taskTypes.every(task => config.taskTypes.includes(task)))
  ));
  const compatibleFlowSchemes = compatibleSchemeConfigs.filter(config => config.type === "流程模板");
  const compatibleMetricSchemes = compatibleSchemeConfigs.filter(config => config.type === "指标方案");
  const selectedFlowConfig = availableSchemeConfigs.find(config => config.type === "流程模板" && config.name === flowScheme);
  const selectedMetricConfig = availableSchemeConfigs.find(config => config.type === "指标方案" && config.name === metricScheme);
  ...
  const applyFlowScheme = (value: string, initializeScope = false) => {
    ...
    if (!value) {
      ...
      if (!metricScheme) applyMetricSelection(modelType && taskTypes.length ? recommendedMetrics(modelType, taskTypes) : []);
      return;
    }
    const config = availableSchemeConfigs.find(item => item.type === "流程模板" && item.name === value);
    if (!config) return;
    if (initializeScope) {
    ...
    setMaxTokens(""); setTemperature(""); setTopK(""); setBatchSize("");
  ...
  const applyMetricScheme = (value: string, initializeScope = false) => {
    setMetricScheme(value);
    if (!value) {
      const fallbackWeights = selectedFlowConfig?.metricWeights;
      if (fallbackWeights && Object.keys(fallbackWeights).length) {
        setMetrics(Object.keys(fallbackWeights));
        setMetricWeights({ ...fallbackWeights });
      } else {
        applyMetricSelection(modelType && taskTypes.length ? recommendedMetrics(modelType, taskTypes) : []);
      }
      return;
    }
    const config = availableSchemeConfigs.find(item => item.type === "指标方案" && item.name === value);
    if (!config) return;
    if (initializeScope) {
      setModelType(config.modelType);
      setTaskTypes(config.taskTypes);
  ...
  useEffect(() => {
    if (initialSchemeConfig?.type === "流程模板" && initialScheme) applyFlowScheme(initialScheme, true);
    if (initialSchemeConfig?.type === "指标方案" && initialScheme) applyMetricScheme(initialScheme, true);
  }, []);
  ...
  const validate = () => {
    ...
    if (batchSize !== "" && (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 128)) next.push("Batch Size 必须为 1～128 的整数");
    setErrors(next);
  ...
  const submit = () => {
    ...
    const created: EvalTask = {
      ...
      createdAt: ts,
      updatedAt: ts,
      scheme: [flowScheme ? `流程模板：${flowScheme}` : "", metricScheme ? `指标方案：${metricScheme}` : ""].filter(Boolean).join("；") || "未使用配置方案",
    };
    onDone(created);
    onClose();
  };
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(780px, 100vw)", background: "#fff", zIndex: 101, boxShadow: "-8px 0 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column" }}>
        <div className="flex items-center justify-between flex-shrink-0" style={{ padding: "14px 20px", borderBottom: "1px solid #f0f2f7" }}>
          <div>
            ...
            <div style={{ fontSize: 12.5, color: "#6b7280", marginTop: 4 }}>选择模型类型后，系统会自动更新可选的评测任务和数据集</div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
          <div style={{ overflow: "auto", padding: "12px 24px 24px", flex: 1, minHeight: 0 }}>
            ...
            <div>
              ...
              <FormRow label="流程模板">
                <select value={flowScheme} onChange={e => applyFlowScheme(e.target.value)} disabled={!modelType || !taskTypes.length} style={{ ...inputSt, background: !modelType || !taskTypes.length ? "#f7f8fa" : "#fff" }}>
                  <option value="">{!modelType || !taskTypes.length ? "请先选择模型类型和评测任务" : "不使用流程模板"}</option>
                  {compatibleFlowSchemes.map(config => <option key={config.name} value={config.name}>{config.name} · {config.version}</option>)}
                </select>
                <div style={{ marginTop: 5, fontSize: 12, color: "#6b7280" }}>可选一个兼容模板，回填执行阶段、阶段参数和流程内默认指标。</div>
              </FormRow>
              <FormRow label="指标方案">
                <select value={metricScheme} onChange={e => applyMetricScheme(e.target.value)} disabled={!modelType || !taskTypes.length} style={{ ...inputSt, background: !modelType || !taskTypes.length ? "#f7f8fa" : "#fff" }}>
                  <option value="">{!modelType || !taskTypes.length ? "请先选择模型类型和评测任务" : "不使用指标方案"}</option>
                  {compatibleMetricSchemes.map(config => <option key={config.name} value={config.name}>{config.name} · {config.version}</option>)}
                </select>
                <div style={{ marginTop: 5, fontSize: 12, color: "#6b7280" }}>可再选一个兼容指标方案；其指标与权重覆盖流程模板中的默认指标。</div>
              </FormRow>
              {(selectedFlowConfig || selectedMetricConfig) && (
                <FormRow label="方案应用结果">
                  <div style={{ padding: "10px 12px", background: "#f7f9ff", borderLeft: "3px solid #4f6ef7", borderRadius: 4, fontSize: 12.5, color: "#374151", lineHeight: 1.75 }}>
                    ...
                    {selectedMetricConfig && <div><b>指标方案：</b>{selectedMetricConfig.name} · {selectedMetricConfig.version}</div>}
                    <div><b>评估指标：</b>{appliedMetricSummary || "未配置"}</div>
                    <div style={{ marginTop: 3, color: "#6b7280" }}>
                      模型与数据集需继续选择。{selectedMetricConfig ? "指标方案覆盖流程模板中的默认指标与权重，保留指标计算阶段的条件规则和自定义逻辑。" : "当前使用流程模板中的默认指标。"}
                    </div>
                  </div>
                </FormRow>
              )}
              ...
              {!selectedFlowConfig ? (
                <div style={{ padding: "8px 0", borderTop: "1px solid #eef0f5" }}>
                  ...
                  {advancedOpen && (
                    <div style={{ padding: "6px 0 8px" }}>
                      <div style={{ marginBottom: 10, fontSize: 12, color: "#6b7280" }}>以下字段均非必填；留空则跟随所选模型默认配置，填写后仅覆盖对应字段。</div>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "12px 16px" }}>
                        {[
                          ...
                          { key: "batchSize" as const, label: "Batch Size", min: 1, max: 128, step: 1, value: batchSize, hint: "1～128 的整数；数值越大，显存占用越高。" },
                        ].map(field => (
                ...
                <div style={{ padding: "4px 0", borderTop: "1px solid #eef0f5" }}>
                  <div style={{ fontSize: 12, color: "#6b7280", margin: "8px 0" }}>模型推理参数（留空则使用方案默认值，填写后覆盖对应字段）</div>
                  {[
                    { key: "maxTokens" as const, label: "最大 Token", min: 1, step: 1, hint: "正整数，控制单次最大输出长度" },
                    { key: "temperature" as const, label: "Temperature", min: 0, max: 2, step: 0.1, hint: "0～2，数值越低输出越稳定" },
                    { key: "topK" as const, label: "Top-K", min: 1, max: 100, step: 1, hint: "1～100 的整数" },
                    { key: "batchSize" as const, label: "Batch Size", min: 1, max: 128, step: 1, hint: "1～128 的整数" },
                  ].map(field => (
                    <div key={field.key} style={{ display: "grid", gridTemplateColumns: "118px minmax(0, 1fr)", columnGap: 14, alignItems: "center", padding: "5px 0" }}>
                      <div style={{ fontSize: 12.5, fontWeight: 500, color: "#374151" }}>{field.label}</div>
                      <input type="number" min={field.min} max={field.max} step={field.step}
                        value={field.key === "maxTokens" ? maxTokens : field.key === "temperature" ? temperature : field.key === "topK" ? topK : batchSize}
                        placeholder="留空使用方案默认值"
                        onChange={event => updateInferenceOverride(field.key, event.target.value === "" ? "" : Number(event.target.value))}
                        style={{ ...inputSt, height: 32 }} />
                    </div>
                  ))}
                </div>
              )}
...
function TaskDetailPage({ task, onClose, onStop, initialTab = "overview" }: { task: EvalTask; onClose: () => void; onStop: (taskId: string) => void; initialTab?: "overview" | "monitor" | "result" | "report" }) {
  ...
  const logLines = [
    ...
    task.status === "失败" ? `[${task.updatedAt}] ERROR：模型推理失败，${task.failureReason}` : `[${task.updatedAt}] 指标计算：指标任务已提交，报告生成状态正常`,
  ];
  const visibleSamples = SAMPLE_RESULTS.filter(sample => !sampleKeyword || `${sample.id} ${sample.prompt} ${sample.output}`.toLowerCase().includes(sampleKeyword.toLowerCase())).filter(sample => !sampleVerdict || sample.verdict === sampleVerdict);
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 16 }}>
              <div style={{ ...panelSt, padding: 16 }}>
                <div className="flex items-center justify-between" style={{ marginBottom: 14 }}>
                  ...
                  <div className="flex items-center gap-2"><SecondaryButton onClick={() => setLogsPaused(value => !value)}>{logsPaused ? "继续自动滚动" : "暂停自动滚动"}</SecondaryButton>{(task.status === "运行中" || task.status === "排队中") && <SecondaryButton onClick={() => onStop(task.id)}>停止</SecondaryButton>}</div>
                </div>
                ...
                <div style={{ marginBottom: 18 }}>
                  <div className="flex items-center justify-between" style={{ marginBottom: 8, fontSize: 13 }}>
                    <span style={{ color: "#6b7280" }}>当前阶段：{task.stage}</span>
                    <span style={{ color: "#1a1d23", fontWeight: 600 }}>{task.progress}%</span>
                  </div>
                  <ProgressBar value={task.progress} />
                </div>
                <div style={{ background: "#111827", color: "#d1d5db", borderRadius: 8, padding: 14, height: 260, fontFamily: "monospace", fontSize: 12.5, lineHeight: 1.8, overflowY: "auto" }}>
          ...
          {tab === "result" && (
            task.status === "成功" ? (
              <div style={{ ...panelSt, overflow: "hidden" }}>
                ...
                {resultTab === "metrics" ? <div style={{ padding: 14, display: "grid", gap: 12 }}>
                  ...
                  <div style={{ border: "1px solid #e8ebf2", borderRadius: 8, padding: 13 }}>
                    <div className="flex items-center justify-between" style={{ marginBottom: 10 }}>
                      ...
                      <div className="flex items-center gap-2">
                        <button title="缩小图表" onClick={() => setBarScale(v => Math.max(0.8, v - 0.1))} style={{ border: "none", background: "none", color: "#6b7280", cursor: "pointer" }}><ZoomOut size={14} /></button>
                        <span style={{ fontSize: 11.5, color: "#6b7280" }}>{Math.round(barScale * 100)}%</span>
                        <button title="放大图表" onClick={() => setBarScale(v => Math.min(1.4, v + 0.1))} style={{ border: "none", background: "none", color: "#6b7280", cursor: "pointer" }}><ZoomIn size={14} /></button>
                    ...
                    <div ref={barScrollRef} style={{ overflowX: "auto" }}>
                      <div style={{ width: `${Math.max(100, Math.round(barScale * 100))}%`, minWidth: barScale > 1 ? 720 : 0 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 280px", gap: 12 }}>
                    <div style={{ border: "1px solid #e8ebf2", borderRadius: 8, padding: 13 }}>
                      <div className="flex items-center justify-between" style={{ marginBottom: 10 }}>
                        ...
                        <div className="flex items-center gap-2">
                          <button title="缩小图表" onClick={() => setLineScale(v => Math.max(0.8, v - 0.1))} style={{ border: "none", background: "none", color: "#6b7280", cursor: "pointer" }}><ZoomOut size={14} /></button>
                          <span style={{ fontSize: 11.5, color: "#6b7280" }}>{Math.round(lineScale * 100)}%</span>
                          <button title="放大图表" onClick={() => setLineScale(v => Math.min(1.4, v + 0.1))} style={{ border: "none", background: "none", color: "#6b7280", cursor: "pointer" }}><ZoomIn size={14} /></button>
                      ...
                      <div ref={lineScrollRef} style={{ overflowX: "auto" }}>
                        <div style={{ width: `${Math.max(100, Math.round(lineScale * 100))}%`, minWidth: lineScale > 1 ? 720 : 0 }}>
                          <ResponsiveContainer width="100%" height={200}>
...
export function EvaluationConfigPage() {
  const [tab, setTab] = useState<"flow" | "metric">("flow");
  const [templates, setTemplates] = useState<EvaluationScheme[]>(() => readEvaluationSchemeCatalog().templates);
  const [schemes, setSchemes] = useState<EvaluationScheme[]>(() => readEvaluationSchemeCatalog().schemes);
  const [showCreate, setShowCreate] = useState(false);
  ...
  const [metricWeights, setMetricWeights] = useState<Record<string, number>>({});
  const rows = tab === "flow" ? templates : schemes;

  useEffect(() => {
    evaluationSchemeCatalog = {
      templates: templates.map(cloneEvaluationScheme),
      schemes: schemes.map(cloneEvaluationScheme),
    };
  }, [templates, schemes]);
  const metricLibrary = [
  ...
  const create = () => {
    const name = draftName.trim() || (tab === "flow" ? `新建流程模板_${templates.length + 1}` : `新建指标方案_${schemes.length + 1}`);
    if (!draftModelType) return;
    const existing = rows.find(item => item.name === editingName);
    const tasks = existing?.tasks || (draftModelType === "语言模型" ? "文本理解、逻辑推理、问答" : "图文描述、视觉问答、文档解析");
    ...
    if (tab === "flow") {
      ...
      const content: SchemeContent = { modelType: draftModelType, tasks, stages, scope, flowStages: savedFlowStages };
      const next: EvaluationScheme = {
        name, type: "流程模板", ...content, version, author, sharedAccess,
        history: [{ ...content, version, date, operator: "admin", summary: existing ? "编辑并保存流程配置" : "初始版本" }, ...(existing?.history || [])],
      };
      ...
      const stages = metricWeightSummary(savedWeights);
      const content: SchemeContent = { modelType: draftModelType, tasks, stages, scope, metricWeights: savedWeights };
      const next: EvaluationScheme = {
        name, type: "指标方案", ...content, version, author, sharedAccess,
        history: [{ ...content, version, date, operator: "admin", summary: existing ? "编辑并保存指标与权重" : "初始版本" }, ...(existing?.history || [])],
      };
      setSchemes(prev => editingName ? prev.map(item => item.name === editingName ? next : item) : [next, ...prev]);
    }
    setShowCreate(false);
    setEditingName(null);
    setDraftName("");
  ...
  const openEdit = (row: EvaluationScheme) => {
    ...
    if (row.type === "流程模板") {
      setFlowStages(cloneFlowStages(row.flowStages || createDefaultFlowStages(false, "Accuracy")));
      setMetricWeights({});
    } else {
  ...
  const saveSharing = () => {
    ...
    setSchemes(current => current.map(update));
    setShareScheme(null);
    setMessage("共享权限已更新");
  };
  ...
  const rollbackScheme = (name: string, targetVersion: string) => {
    ...
    setSchemes(current => current.map(restore));
    setHistoryName(null);
    setMessage("已按历史内容生成新版本");
  };
  const totalWeight = Object.values(metricWeights).reduce((sum, value) => sum + value, 0);
  const selectedMetricNames = Object.entries(metricWeights).filter(([, weight]) => weight > 0).map(([name]) => name);
  ...
  const editingScheme = editingName ? rows.find(item => item.name === editingName) : null;
    ...
    <WorkbenchPage title="配置方案" crumb="配置方案">
      <div style={{ ...panelSt, display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        <div className="flex items-center justify-between" style={{ padding: "14px 16px", borderBottom: "1px solid #f0f2f7" }}>
          <ToggleGroup options={["流程模板", "指标方案"]} value={tab === "flow" ? "流程模板" : "指标方案"} onChange={v => setTab(v === "流程模板" ? "flow" : "metric")} />
          <div className="flex items-center gap-3">{message && <span style={{ fontSize: 12.5, color: "#16a34a" }}>{message}</span>}<PrimaryButton onClick={openCreate}><Plus size={14} />{tab === "flow" ? "新建流程模板" : "新建指标组合方案"}</PrimaryButton></div>
        </div>
            ...
            <tbody>
              {rows.map(row => {
                return <tr key={row.name}>
                  ...
                  <td style={tdSt}><div>{row.modelType}</div><div style={{ marginTop: 3, color: "#6b7280", fontSize: 11.5 }}>{row.tasks}</div></td>
                  <td style={{ ...tdSt, overflowWrap: "anywhere" }}>{row.stages}</td>
                  ...
                  <td style={{ ...tdSt, position: "sticky", right: 0, background: "#fff", boxShadow: "-1px 0 #f0f2f7" }}><div className="flex items-center" style={{ flexWrap: "wrap", gap: "5px 10px" }}>
                    ...
                    {row.author === CURRENT_USER && <TextButton danger onClick={() => tab === "flow" ? setTemplates(prev => prev.filter(item => item.name !== row.name)) : setSchemes(prev => prev.filter(item => item.name !== row.name))}>删除</TextButton>}
                  </div></td>
                </tr>;
              })}
            </tbody>
      ...
      {showCreate && (
        <>
          ...
          <div style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: "min(560px, 100vw)", background: "#fff", zIndex: 101, boxShadow: "-8px 0 32px rgba(0,0,0,0.12)", display: "flex", flexDirection: "column" }}>
            <div className="flex items-center justify-between" style={{ padding: "14px 18px", borderBottom: "1px solid #f0f2f7" }}>
              <div style={{ fontSize: 16, fontWeight: 600 }}>{editingName ? "编辑" : "创建"}{tab === "flow" ? "流程模板" : "指标组合方案"}</div>
            ...
            <div style={{ padding: 18, flex: 1, overflow: "auto" }}>
              ...
              <div style={{ marginBottom: 14 }}><FieldLabel required>适用模型类型</FieldLabel><select value={draftModelType} onChange={e => setDraftModelType(e.target.value as ModelType | "")} style={inputSt}><option value="">请选择模型类型</option><option>语言模型</option><option>多模态模型</option></select></div>
              {tab === "flow" ? (
                <div style={{ marginBottom: 14 }}>
                  <FieldLabel required>执行阶段与参数</FieldLabel>
                  <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>数据预处理必须在模型推理之前；指标计算必须在模型推理之后；后处理可跳过。</div>
                    ...
                    return <div key={stage.name} style={{ border: `1px solid ${flowError ? "#fecaca" : "#e8ebf2"}`, borderRadius: 8, padding: 10, marginBottom: 8 }}>
                      ...
                      {stage.enabled && stage.name === "模型推理" && (
                        <div style={{ marginTop: 8, borderTop: "1px solid #eef0f5", paddingTop: 6 }}>
                          <div style={{ fontSize: 11.5, color: "#6b7280", marginBottom: 6 }}>模型推理参数（留空则使用默认配置，填写后覆盖对应字段）</div>
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 }}>
                            {[
                              { label: "最大 Token", key: "maxTokens", min: 1, step: 1, hint: "正整数，不得超过模型支持上限。" },
                              { label: "Temperature", key: "temperature", min: 0, max: 2, step: 0.1, hint: "0～2，建议使用 0.1～1.0。" },
                              { label: "Top-K", key: "topK", min: 1, max: 100, step: 1, hint: "1～100 的整数，越小候选越集中。" },
                              { label: "Batch Size", key: "batchSize", min: 1, max: 128, step: 1, hint: "1～128 的整数，越大显存占用越高。" },
                            ].map(field => (
                      ...
                      {stage.enabled && stage.name === "指标计算" && <div style={{ marginTop: 8 }}><div style={{ fontSize: 11.5, color: "#6b7280", marginBottom: 5 }}>指标组合</div><div className="flex items-center gap-3">{["Accuracy", "F1", "ROUGE"].map(metric => <label key={metric} className="flex items-center gap-1" style={{ fontSize: 12 }}><input type="checkbox" checked={selectedMetrics.includes(metric)} onChange={e => updateStageParam(stage.name, "metrics", (e.target.checked ? [...selectedMetrics, metric] : selectedMetrics.filter(item => item !== metric)).join(","))} />{metric}</label>)}</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}><label style={{ fontSize: 11.5, color: "#6b7280" }}>条件规则<input value={String(stage.params.conditionRule)} onChange={e => updateStageParam(stage.name, "conditionRule", e.target.value)} style={{ ...inputSt, marginTop: 4 }} /></label><label style={{ fontSize: 11.5, color: "#6b7280" }}>自定义评估逻辑<input value={String(stage.params.customLogic)} onChange={e => updateStageParam(stage.name, "customLogic", e.target.value)} style={{ ...inputSt, marginTop: 4 }} /></label></div></div>}
                    </div>;
                  })}
                ...
                <div style={{ marginBottom: 14 }}>
                  <FieldLabel required>指标与权重</FieldLabel>
                  {["生成", "分类", "匹配", "代码生成", "效率"].map(category => (
                    <div key={category} style={{ marginBottom: 10 }}>
                      ...
                      {metricLibrary.filter(metric => metric.category === category).map(metric => (
                        <div key={metric.name} style={{ display: "grid", gridTemplateColumns: "22px 88px minmax(0, 1fr) 72px", gap: 8, alignItems: "center", borderBottom: "1px solid #f0f2f7", padding: "9px 0", fontSize: 12.5 }}>
                          <input type="checkbox" checked={(metricWeights[metric.name] || 0) > 0} onChange={e => setMetricWeights(equalMetricWeights(e.target.checked ? [...selectedMetricNames, metric.name] : selectedMetricNames.filter(name => name !== metric.name)))} />
                          <b>{metric.name}</b>
                          ...
                          <input aria-label={`${metric.name} 权重`} type="number" min={0} max={100} disabled={(metricWeights[metric.name] || 0) <= 0} value={(metricWeights[metric.name] || 0) > 0 ? metricWeights[metric.name] : ""} placeholder="-" onChange={e => setMetricWeights(prev => ({ ...prev, [metric.name]: Number(e.target.value) }))} style={{ ...inputSt, height: 30 }} />
                        </div>
                      ))}
                    </div>
                  ))}
                  <div style={{ marginTop: 10, color: totalWeight === 100 ? "#16a34a" : "#dc2626", fontSize: 12.5 }}>当前权重合计：{totalWeight}%（保存时应为 100%）。系统按权重计算加权总体得分。</div>
                </div>
              )}
            ...
            <div className="flex items-center justify-end gap-2" style={{ padding: "12px 18px", borderTop: "1px solid #f0f2f7" }}>
              ...
              <PrimaryButton disabled={!draftName.trim() || !draftModelType || (tab === "metric" && totalWeight !== 100) || (tab === "flow" && (Boolean(flowError) || flowParamsIncomplete))} onClick={create}>保存</PrimaryButton>
            </div>
          </div>
        </>
      )}
      ...
      {historyName && (() => {
        const selectedScheme = [...templates, ...schemes].find(item => item.name === historyName);
      ...
      {applySchemeName && (() => {
        const selectedScheme = [...templates, ...schemes].find(item => item.name === applySchemeName);
