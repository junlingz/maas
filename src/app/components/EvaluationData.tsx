...
type MyDatasetPermission = Exclude<DatasetRow["permission"], "公开">;
...
const INITIAL_DATASETS: DatasetRow[] = [
  ...
  { id: 3, name: "GSM8K", description: "小学数学应用题的多步推理评测集。", source: "public", modelType: "语言模型", tasks: ["逻辑推理", "问答"], format: "内置", count: 8792, domain: "数学推理", version: "v1.1", versions: ["v1.1", "v1.0"], recommendedVersion: "v1.1", status: "校验通过", permission: "公开", creator: "system", team: "系统", updatedAt: "2026-03-20 18:10:03", metrics: ["Accuracy", "关键词匹配率"], schema: "id,input,answer,rationale", sample: "{A shop sells 12 apples...}", citation: "Cobbe et al., Training Verifiers to Solve Math Word Problems", versionDetails: { "v1.1": { count: 8792, sample: "{\"A shop sells 12 apples...\"}", schema: "id,input,answer,rationale", citation: "Cobbe et al., Training Verifiers to Solve Math Word Problems, 2021", updatedAt: "2026-03-20", releaseNote: "补充 1300+ 道多步推理题；增加解题步骤标注。" }, "v1.0": { count: 7473, sample: "{\"If John has 5 apples...\"}", schema: "id,input,answer", citation: "Cobbe et al., Training Verifiers to Solve Math Word Problems, 2021", updatedAt: "2025-05-12", releaseNote: "GSM8K 数据集初始版本。" } } },
  ...
  { id: 8, name: "政务问答共享集", description: "共享的政务服务问答数据集。", source: "shared", modelType: "语言模型", tasks: ["问答"], format: "CSV", count: 5200, domain: "政务", version: "v2", versions: ["v2", "v1"], recommendedVersion: "v2", status: "校验通过", permission: "团队可见", creator: "张小明", team: "AI研发团队", updatedAt: "2026-07-16 15:18:00", metrics: ["Accuracy", "F1"], schema: "id,input,answer", sample: "{如何办理业务材料？}", citation: "内部数据集，无公开引用", versionDetails: { "v2": { count: 5200, sample: "{\"如何办理业务材料？\"}", schema: "id,input,answer", updatedAt: "2026-07-16", releaseNote: "新增 1400 条社保与户籍业务数据；统一问答格式。" }, "v1": { count: 3800, sample: "{\"业务办理需要什么？\"}", schema: "input,answer", updatedAt: "2026-06-05", releaseNote: "初始版本。" } } },
...
function UploadDatasetPage({ onBack, onDone, uploadMode = "mine" }: { onBack: () => void; onDone: (row: DatasetRow) => void; uploadMode?: "mine" | "public" }) {
  ...
  const [uploadPermission, setUploadPermission] = useState<MyDatasetPermission>("仅自己可见");
  const [sample, setSample] = useState("");
  const [domain, setDomain] = useState("自定义");
  const [citation, setCitation] = useState("内部数据集，无公开引用");
  const resolvedUploadPermission: DatasetRow["permission"] = uploadMode === "public" ? "公开" : uploadPermission;
  ...
  const submit = () => {
    ...
    onDone({ id: Date.now(), name: name.trim(), description: description.trim(), source: uploadMode, modelType, tasks: [task], format, count: recordCount, domain: domain.trim() || "自定义", version: "v1", versions: ["v1"], recommendedVersion: "v1", status: validation || "校验中", permission: resolvedUploadPermission, creator: "admin", team: uploadMode === "public" ? "系统" : "公共", updatedAt: new Date().toLocaleString("zh-CN", { hour12: false }), metrics: getSuggestedMetrics([task]), schema: modelType === "多模态模型" ? "image_url,question,answer" : task === "代码生成" ? "prompt,test" : "input,answer", sample: sample || fileName, citation: citation.trim() || "内部数据集，无公开引用", validationError: validation === "校验失败" ? error : undefined });
    onBack();
  };
    ...
    <div className="flex-1 min-h-0" style={{ margin: "10px 18px 18px", background: "#fff", border: "1px solid #e8ebf2", borderRadius: 8, overflow: "auto" }}>
      ...
      <div style={{ padding: "0 20px 18px" }}>
        <main>
          ...
          {uploadMode === "mine" && <section id="dataset-permission" style={{ padding: "18px 0", borderBottom: "1px solid #eef1f6" }}><h3 style={{ margin: "0 0 13px", fontSize: 14 }}>权限设置</h3><div><FieldLabel>权限范围</FieldLabel><select aria-label="权限范围" value={uploadPermission} onChange={e => setUploadPermission(e.target.value as MyDatasetPermission)} style={inputSt}><option value="仅自己可见">仅自己可见</option><option value="团队可见">团队可见</option><option value="团队可编辑">团队可编辑</option></select>{uploadPermission !== "仅自己可见" && <div role="alert" style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8, color: "#dc2626", fontSize: 12, fontWeight: 600 }}><AlertCircle size={14} style={{ flexShrink: 0 }} />共享至团队后，不可改回仅自己可见，请谨慎修改。</div>}</div></section>}
...
function ShareDrawer({ row, onClose, onSave }: { row: DatasetRow; onClose: () => void; onSave: (permission: MyDatasetPermission) => void }) {
  const alreadyShared = row.permission !== "仅自己可见";
  const initialPermission: MyDatasetPermission = row.permission === "公开" ? "仅自己可见" : row.permission;
  const [permission, setPermission] = useState<MyDatasetPermission>(initialPermission);
  return <><div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", zIndex: 120 }} /><div style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: "min(440px, 100vw)", boxSizing: "border-box", background: "#fff", zIndex: 121, padding: 22 }}><div className="flex items-center justify-between"><b>数据集权限设置</b><button aria-label="关闭权限设置" onClick={onClose} style={{ border: "none", background: "none", cursor: "pointer" }}><X size={18} /></button></div><p style={{ color: "#6b7280", fontSize: 13 }}>{row.name}</p><FieldLabel>权限范围</FieldLabel><select aria-label="权限范围" value={permission} onChange={e => setPermission(e.target.value as MyDatasetPermission)} style={inputSt}><option value="仅自己可见" disabled={alreadyShared}>仅自己可见</option><option value="团队可见">团队可见</option><option value="团队可编辑">团队可编辑</option></select>{permission !== "仅自己可见" && <div role="alert" style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8, color: "#dc2626", fontSize: 12, fontWeight: 600 }}><AlertCircle size={14} style={{ flexShrink: 0 }} />共享至团队后，不可改回仅自己可见，请谨慎修改。</div>}<div className="flex items-center justify-end gap-2" style={{ marginTop: 20 }}><SecondaryButton onClick={onClose}>取消</SecondaryButton><PrimaryButton onClick={() => onSave(permission)}>保存</PrimaryButton></div></div></>;
}
